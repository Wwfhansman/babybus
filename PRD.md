...
| 核心漫画生成模块 | 小说输入区 | 文本标记辅助 | 仅在第三级（内容层）显示文本标记按钮：「标记对话」「标记场景」「标记角色动作」，选中文本点击后分别以蓝、绿、橙色高亮。<br>标记信息仅用于当前章节生成漫画时，辅助后端 LLM 精准解析（如对话编入气泡、场景生成背景），不存储到本地小说文本中。<br>支持批量取消标记（选中文本 + 点击“清除标记”）。 |
| 核心漫画生成模块 | 设定编辑区 | 分镜内容展示 | 点击第三级（内容层）「生成漫画」按钮后，后端 LLM 在 30 秒内返回解析结果，设定编辑区仅展示“分镜内容”单一板块，无角色形象编辑功能（角色形象由第一级小说「设置」中配置）。<br>分镜内容按 1-8 话序号排列，每话展示“分镜台词”+“分镜场景描述”：<br>• 分镜台词：提取章节文本中的对话内容，标注说话角色（基于小说角色配置匹配）；<br>• 分镜场景描述：通俗化表述场景与角色动作（如“分镜 1：客厅内，男主递信给女主，女主惊讶接信”），避免专业术语。<br>支持修改分镜台词与场景描述（直接点击文本框编辑），不支持新增/删除分镜（分镜数量由章节字数自动匹配，500-1000 字对应 3-5 话，1000-5000 字对应 6-8 话）。 |
| 核心漫画生成模块 | 设定编辑区 | 排版模板选择 | 设定编辑区底部新增“排版模板选择”栏，提供 4 类模板：<br>• 横版多格（6 格/页，适配公众号）；<br>• 竖版条漫（3 格/页，适配短视频）；<br>• 方形四格（4 格/页，适配社群）；<br>• 自定义格数（2-8 格/页）。<br>选中模板后显示预览图，标注每格尺寸（如竖版：1080×1920px），模板选择后不可修改（需返回重新生成）。 |
